...
def build_everything(args: arg_util.Args):
    '''训练流程的核心初始化函数，负责构建训练所需组件'''
    # resume
    # 检查是否有可恢复的检查点，若有，获取恢复信息、起始epoch、起始iteration、训练器状态、参数状态
    auto_resume_info, start_ep, start_it, trainer_state, args_state = auto_resume(args, 'ar-ckpt*.pth')
    # create tensorboard logger
    tb_lg: misc.TensorboardLogger
    with_tb_lg = dist.is_master() # 当前进程是否为主进程
    if with_tb_lg: # 主进程创建真实的TensorBoard记录器，立即写入
        os.makedirs(args.tb_log_dir_path, exist_ok=True) # create log dir
    ...
    if not args.local_debug: # 非调试模式
        ...
        # 验证集，EvalDistributedSampler确保评估一致性
        ld_val = DataLoader(
        ...
        del dataset_val # 加载后删除原始数据集，内存优化
        
        # 训练集，DistInfiniteBatchSampler实现无限迭代
        ld_train = DataLoader(
        ...
        stt = time.time()
        iters_train = len(ld_train) # return
        ld_train = iter(ld_train) # return
        ...
        # 简化配置，快速验证过程
        num_classes = 1000 # 虚拟数据
        ld_val = ld_train = None
        iters_train = 10 # 少量迭代
    
    # build models
    from torch.nn.parallel import DistributedDataParallel as DDP
    from models import VAR, VQVAE, build_vae_var # customed
    from trainer import VARTrainer # customed
    from utils.amp_sc import AmpOptimizer # customed
    from utils.lr_control import filter_params # customed
    ...
    # load vae model's pretraining weights
    vae_ckpt = 'vae_ch160v4096z32.pth' 
    if dist.is_local_master():
        if not os.path.exists(vae_ckpt):
            os.system(f'wget https://hf-mirror.com/FoundationVision/var/resolve/main/{vae_ckpt}')
    dist.barrier() # 进程同步，确保所有进程下载完再加载
    vae_local.load_state_dict(torch.load(vae_ckpt, map_location='cpu'), strict=True)
    
    # compile vae model
    vae_local: VQVAE = args.compile_model(vae_local, args.vfast) # local
    var_wo_ddp: VAR = args.compile_model(var_wo_ddp, args.tfast) # distribute data parallel
    # 单机：NullDDP 分布式：DDP
    ...
    count_p = lambda m: f'{sum(p.numel() for p in m.parameters())/1e6:.2f}' # 统计各组件参数量，单位是MB
    ...
    # build optimizer
    # filter parameters, sepcial tokens are excluded
    ...
    var_optim = AmpOptimizer(
        ...
        grad_clip=args.tclip, # 梯度裁剪
        n_gradient_accumulation=args.ac # 梯度累计步数
    )
    ...
    trainer = VARTrainer(
        ...
        resos=args.resos,
        vae_local=vae_local, # image Encoder
        var_wo_ddp=var_wo_ddp, # 主体模型（无DDP）
        var=var, # 分布式模型
        var_opt=var_optim, # 优化器
        label_smooth=args.ls, # 标签平滑系数
    )
    if trainer_state is not None and len(trainer_state):
        trainer.load_state_dict(trainer_state, strict=False, skip_vae=True) # don't load vae again
    del vae_local, var_wo_ddp, var, var_optim
    
    if args.local_debug: # 调试模式
        ...
        ) # prog_si=-1 means not doing progressive training
        print({k: meter.global_avg for k, meter in me.meters.items()})
        
        # print metrics and clean
...
def main_training():
    # 环境初始化
    # init_dist_and_get_args完成命令行args参数解析、分布式训练、GPU绑定
    args: arg_util.Args = arg_util.init_dist_and_get_args() 
    if args.local_debug: # debug mode
        torch.autograd.set_detect_anomaly(True) # 启用pytorch的异常梯度检测，便于发现 NAN/INF问题
    
    # 组件构建
    # 六大核心组件：日志记录器 训练器 起始epoch 起始iter one ep的训练次数 训练集长度 验证集长度
    (
        tb_lg, trainer,
        start_ep, start_it,
        iters_train, ld_train, ld_val
    ...
    # training loop
    for ep in range(start_ep, args.ep): # 从起始ep—最大ep
        # 设置数据加载器epoch（分布式必需）
        ...
        if is_val_and_also_saving:
            ...
            best_updated = best_val_loss_tail > val_loss_tail # Lt更小时，更新
            ...
            if dist.is_local_master(): # 主进程保存检查点
                # 双检查点：最近+最佳
                ...
                if best_updated:
                # 保存最佳模型，因为当前模型就是最佳，所以只需copy刚刚保存的检查点副本即可，节省时间
                    shutil.copy(local_out_ckpt, local_out_ckpt_best) 
                print(f'     [saving ckpt](*) finished!  @ {local_out_ckpt}', flush=True, clean=True)
            dist.barrier() # 进程同步，等待主进程保存好检查点
    ...
    time.sleep(3), gc.collect(), torch.cuda.empty_cache(), time.sleep(3) # 为回收留出缓冲时间
    
    args.remain_time, args.finish_time = '-', time.strftime("%Y-%m-%d %H:%M", time.localtime(time.time() - 60))
    print(f'final args:\n\n{str(args)}')
    # 将当前所有配置参数（param lr batch_size training_time）持久化写入日志
    args.dump_log(); tb_lg.flush(); tb_lg.close()
    dist.barrier()
...
def train_one_ep(ep: int, is_first_ep: bool, start_it: int, args: arg_util.Args, tb_lg: misc.TensorboardLogger, ld_or_itrt, iters_train: int, trainer):
    ...
    # 初始化日志记录器，SmoothedValue是平滑值计算器，window_size=1说明不应用平滑
    me = misc.MetricLogger(delimiter='  ')
    me.add_meter('tlr', misc.SmoothedValue(window_size=1, fmt='{value:.2g}')) # 动态学习率
    me.add_meter('tnm', misc.SmoothedValue(window_size=1, fmt='{value:.2f}')) # 梯度范数
    [me.add_meter(x, misc.SmoothedValue(fmt='{median:.3f} ({global_avg:.3f})')) for x in ['Lm', 'Lt']] #平均学习率 尾部学习率
    [me.add_meter(x, misc.SmoothedValue(fmt='{median:.2f} ({global_avg:.2f})')) for x in ['Accm', 'Acct']] # 平均准确率 尾部准确率
    header = f'[Ep]: [{ep:4d}/{args.ep}]'
    
    # 若是第一轮，忽略某些警告
    if is_first_ep:
        warnings.filterwarnings('ignore', category=DeprecationWarning) # 弃用警告
        warnings.filterwarnings('ignore', category=UserWarning) # 用户警告
    g_it, max_it = ep * iters_train, args.ep * iters_train # 全局迭代次数 最大迭代次数
    
    # 遍历数据集
    for it, (inp, label) in me.log_every(start_it, iters_train, ld_or_itrt, 30 if iters_train > 8000 else 5, header):
        g_it = ep * iters_train + it # 计算全局迭代次数（当前迭代次数）
        if it < start_it: continue # 跳过已处理的迭代
        if is_first_ep and it == start_it: warnings.resetwarnings() # 充值过滤警告
        
        inp = inp.to(args.device, non_blocking=True) # 移动数据inp到指定设备
        label = label.to(args.device, non_blocking=True) # 移动标签label到指定设备
        
        args.cur_it = f'{it+1}/{iters_train}' # 在这个epoch中的训练轮数/总轮数
        
        wp_it = args.wp * iters_train # 预热阶段
        # 动态调整学习率 权重衰减系数
        # min_tlr max_tlr: 最小学习率 最大学习率
        # min_twd max_twd: 最小权重衰减 最大权重衰减
        ...
        args.cur_lr, args.cur_wd = max_tlr, max_twd # 更新lr wd
        
        # 判断是否使用渐进式训练progressive training，计算训练阶段索引数prog_si
        if args.pg: # default: args.pg == 0.0, means no progressive training, won't get into this
            if g_it <= wp_it: prog_si = args.pg0 # 预热阶段，使用初始阶段
            elif g_it >= max_it*args.pg: prog_si = len(args.patch_nums) - 1 # 大于最大迭代次数，最终阶段
            else: # 根据当前迭代次数计算prog_si，它决定了使用哪个patch
        ...
        stepping = (g_it + 1) % args.ac == 0 # 是否进行梯度更新
        # 原本ac=1，stepping一直都是true
        #print(f"*********g_it: {g_it}")
        #print(f"*********ep: {ep}")
        #print(f"*********iters_train: {iters_train}")
        #print(f"*********it: {it}")
        
        step_cnt += int(stepping) # 累加梯度更新次数
    ...
    me.synchronize_between_processes() # 在多个进程中保持日志记录器的同步
