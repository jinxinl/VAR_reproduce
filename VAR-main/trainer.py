...
class VARTrainer(object):
    ...
    def eval_ep(self, ld_val: DataLoader):
        ...
        for inp_B3HW, label_B in ld_val:
            ...
            gt_idx_Bl: List[ITen] = self.vae_local.img_to_idxBl(inp_B3HW) # img -> token
            gt_BL = torch.cat(gt_idx_Bl, dim=1) # concatenate on channel
            x_BLCv_wo_first_l: Ten = self.quantize_local.idxBl_to_var_input(gt_idx_Bl) # quantize
            
            self.var_wo_ddp.forward # 1.预热计算图 2.验证输入输出维度一致
    ...
    def train_step(
        ...
        self.prog_it += 1
        prog_wp = max(min(self.prog_it / prog_wp_it, 1), 0.01) # 渐进式热身系数，随着训练进行不断变大，阈值0.01控制，实现平滑过渡，防止loss突变
        if self.first_prog: prog_wp = 1    # no prog warmup at first prog stage, as it's already solved in wp
        if prog_si == len(self.patch_nums) - 1: prog_si = -1    # max prog, as no prog
        ...
        with self.var_opt.amp_ctx: # amp_ctx：自动混合精度上下文，自动管理FP16/FP32的转换
            ...
            if prog_si >= 0:    # in progressive training
                ...
                lw[:, bg:ed] *= min(max(prog_wp, 0), 1) # 应用热身系数实现注意力引导，关注当前分辨率特征，并实现平滑过渡
